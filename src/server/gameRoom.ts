...
export class GameRoom extends Room {
    ...
    private userIdsAndUsernames: PB_MessageToClient_Game_UserIdAndUsername[] =
        [];
    private clientToNumberOfGameStatesPerPlayerAndWatcher = new WeakMap<
        Client,
        number[]
    >();
    ...
    onMessage_Connect(
        client: Client,
        message: PB_MessageToServer_Game_Connect,
    ) {
        ...
        const numberOfGameStatesPerPlayerAndWatcher = message
            .numberOfGameStatesPerPlayerAndWatcher.slice(0, numEntriesRequired);
        for (
            let i = numberOfGameStatesPerPlayerAndWatcher.length;
            i < numEntriesRequired;
            i++
        ) {
        ...
        const messageToClient = PB_MessageToClient.create({
            game: {
                connectResponse: {
                    ...
                    metadata:
                        numberOfGameStatesPerPlayerAndWatcher.every((entry) =>
                                entry === 0
                            )
                            ? {
                                gameMode: this.gameSetup
                                ...
                                hostUserId: this.gameSetup
                                    ? this.gameSetup.hostUser.id
                                    : this.game!.hostUser.id,
                                ...
                                approvals: this.gameSetup
                                    ? this.gameSetup.approvals
                                    : dummyApprovals,
                                numberOfGameSetupChanges: this.gameSetup
                                    ? this.numberOfGameSetupChanges
                                    : 0,
                            }
                            : undefined,
                    userIdsInRoom: [...this.userToClients.keys()].map((user) =>
                        user.id
                    ),
                },
                userIdsAndUsernames:
                    message.numberOfUserIdAndUsernameMessages === 0
                        ...
                        : this.userIdsAndUsernames.slice(
                            message.numberOfUserIdAndUsernameMessages,
                        ),
            },
        });

        const gameStateMessages = this.gameStateMessagesClientDoesNotHave(
            client,
        );
    ...
    onMessage_GameSetupAction(
        client: Client,
        message: PB_MessageToServer_Game_GameSetupAction,
    ) {
        ...
        if (message.sitDown) {
            ...
            if (
                this.gameSetup.history[0]?.userApprovedOfGameSetup
                    ?.approvedByEverybody
            ) {
                queueLobbyEvent = true;
            }
            ...
            if (client.user === this.gameSetup.hostUser) {
                const botGeneration = this.botGenerator(
                    message.addBot?.botType,
                    this,
                );
        ...
        if (this.gameSetup.finalUsers) {
            ...
            this.game.doGameAction(
                PB_GameAction.create({ startGame: {} }),
                this.dateNow(),
            );

            for (const client of this.clients) {
                const initialNumberOfGameStatesPerPlayerAndWatcher = new Array(
                    this.game.users.length + 1,
                );
    ...
    onMessage_GameAction(
        client: Client,
        message: PB_MessageToServer_Game_GameAction,
    ) {
        if (
            !this.game ||
            !client.user ||
        ...
        const currentAction =
            this.game.gameActionStack[this.game.gameActionStack.length - 1];
    ...
    gameStateMessagesClientDoesNotHave(client: Client) {
        ...
        const numberOfGameStatesPerPlayerAndWatcher = this
            .clientToNumberOfGameStatesPerPlayerAndWatcher.get(client)!;

        const playerId = client.user
            ? this.game.users.indexOf(client.user)
            : -1;
        const numberOfGameStates = numberOfGameStatesPerPlayerAndWatcher[
            playerId === -1 ? this.game.users.length : playerId
        ];
        ...
        for (
            let i = numberOfGameStates;
            i < this.game.gameStateHistory.length;
            i++
        ) {
            const gameState = this.game.gameStateHistory[i];
            gameStateMessages.push(
                playerId >= 0
                    ? gameState.playerGameStates[playerId]
    ...
    userConnected(user: User) {
        ...
        if (!this.userIdToUser.has(user.id)) {
            ...
            const userIdAndUsername = PB_MessageToClient_Game_UserIdAndUsername
                .create({
                    userId: user.id,
                    username: user.name,
                });
    ...
    private sendLastGameStateToClients() {
        ...
        for (
            let playerId = 0;
            playerId < gameState.playerGameStates.length;
            playerId++
        ) {
            ...
            if (clients) {
                ...
                for (const client of clients) {
                    ...
                    this.clientToNumberOfGameStatesPerPlayerAndWatcher.get(
                        client,
                    )![playerId]++;
                }
            }
