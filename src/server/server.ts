...
export class Server {
    ...
    logMessage = (clientId: number, message: Uint8Array) => {
        console.log("==== Message".padEnd(25, "="));
        console.log(`sender id: ${clientId}`);
        const client = this.clientIdToClient.get(clientId)!;
        console.log(`sender name: ${client.user?.name}`);
        const messageToServer = PB_MessageToServer.fromBinary(message);
        if (messageToServer.game) {
            console.log("game message");
            console.log(messageToServer.game);
        } else if (messageToServer.lobby) {
            console.log("lobby message");
            console.log(messageToServer.lobby);
        }
    ...
    private async onMessage_LoginLogout(
        client: Client,
        message: PB_MessageToServer_LoginLogout,
    ) {
        ...
        if (message.loginWithPassword) {
            await this.onMessage_LoginLogout_LoginWithPassword(
                client,
                message.loginWithPassword,
            );
        } else if (message.loginWithToken) {
            await this.onMessage_LoginLogout_LoginWithToken(
                client,
                message.loginWithToken,
            );
        } else if (message.createUserAndLogin) {
            await this.onMessage_LoginLogout_CreateUserAndLogin(
                client,
    ...
    private async onMessage_LoginLogout_LoginWithPassword(
        ...
        const userDataProviderResponse = await this.userDataProvider.lookupUser(
            username,
        );
        ...
        if (userDataProviderResponse.errorCode !== undefined) {
            this.sendLoginLogoutMessage(
                client,
                userDataProviderResponse.errorCode,
            );
            return;
        }
        ...
        if (
            !userDataProviderResponse.userData.verifyPassword(message.password)
        ) {
            this.sendLoginLogoutMessage(
                client,
    ...
    private sendLoginLogoutMessage(
        ...
        client.sendMessage(
            PB_MessageToClient.toBinary(
                createLoginLogoutMessage(responseCode, username, userId, token),
            ),
        );
    }
}
