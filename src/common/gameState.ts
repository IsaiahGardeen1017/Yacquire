...
import { SaneGameState } from "./SaneGameState.js";
...
export class GameState {
    ...
    constructor(
        ...
        this.nextGameAction =
            game.gameActionStack[game.gameActionStack.length - 1];
    }
    ...
    addPlayedTile(tile: number, playerId: number) {
        ...
        if (this.revealedTileBagTilesLookup.has(tile)) {
            // change it to public
            this.revealedTileBagTilesLookup.get(tile)!.playerIdWithPermission =
                null;
        } else {
            // add it to the tile rack additions
            const revealedTileRackTile = PB_GameState_RevealedTileRackTile
                .create({
                    tile,
    ...
    createPlayerAndWatcherGameStates() {
        ...
        for (
            let playerId = 0;
            playerId < this.playerGameStates.length;
            playerId++
        ) {
    ...
    createGameState(playerId: number) {
        ...
        if (this.playerIdWithPlayableTile !== null) {
            gameState.playerIdWithPlayableTilePlusOne =
                this.playerIdWithPlayableTile + 1;
        }
