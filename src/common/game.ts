...
export class Game {
    ...
    constructor(
        ...
        this.gameActionStack.push(
            new ActionStartGame(this, users.indexOf(hostUser)),
        );
    ...
    processGameState(gameState: PB_GameState) {
        ...
        if (gameState.timestamp !== 0) {
            ...
            if (timestamp !== null) {
                const previousGameState = this.gameStateHistory.length > 0
                    ? this.gameStateHistory[this.gameStateHistory.length - 1]
                    : null;
                if (
                    previousGameState !== null &&
                    previousGameState.timestamp !== null
                ) {
        ...
        if (gameState.playerIdWithPlayableTilePlusOne) {
            this.processPlayerIdWithPlayableTile(
    ...
    getSaneGamestateObject(playerIndex: number): SaneGameState {
        const NUM_CHAINS = 7;
        const numPlayers = this.users.length;
        const playerIndexToSaneIndex = (pi: number) => {
            return (pi + (numPlayers - playerIndex)) % numPlayers;
        };
        ...
        for (let i = 0; i < numPlayers; i++) {
            ...
            stocks.push(s);
            playerCash.push(-1);
            playerNetWorths.push(-1);
        }
        for (let i = 0; i < this.scoreBoardAtLastNetWorthsUpdate.length; i++) {
            let idx = playerIndexToSaneIndex(i);
            stocks[idx];
            for (let j = 0; j < NUM_CHAINS; j++) {
    ...
    processRevealedTileRackTiles(entries: PB_GameState_RevealedTileRackTile[]) {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            for (let tileIndex = 0; tileIndex < 6; tileIndex++) {
                if (
                    this.tileRacks[playerIdBelongsTo][tileIndex] ===
                        TileEnum.Unknown
                ) {
                    this.tileRacks[playerIdBelongsTo] = [
                        ...this.tileRacks[playerIdBelongsTo],
                    ];
    ...
    doGameAction(gameAction: PB_GameAction, timestamp: number | null) {
        let currentAction =
            this.gameActionStack[this.gameActionStack.length - 1];
    ...
    drawTiles(playerId: number) {
        const addDrewTileMessage = this.myUser === null ||
            this.myUser === this.users[playerId];
        ...
        for (let tileIndex = 0; tileIndex < 6; tileIndex++) {
            ...
            if (tile !== null && tile !== TileEnum.Unknown) {
                ...
                for (let i = 0; i < neighboringTiles.length; i++) {
                    ...
                    borderTypes.push(
                        this.gameBoard[neighboringTile.y][neighboringTile.x],
                    );
                }

                borderTypes = borderTypes.filter((type, index) => {
                    if (
                        type === PB_GameBoardType.NOTHING ||
                        type === PB_GameBoardType.CANT_PLAY_EVER
                    ) {
                        return false;
                    }
                ...
                if (borderTypes.length > 1) {
                    borderTypes = borderTypes.filter((type) =>
                        type !== PB_GameBoardType.NOTHING_YET
                    );
                }
                ...
                if (borderTypes.length === 0) {
                    ...
                    if (
                        borderTypes.indexOf(PB_GameBoardType.NOTHING_YET) !== -1
                    ) {
                        if (canStartNewChain) {
    ...
    getScoreBoardColumnArray(
        scoreBoardIndex: PB_GameBoardType | ScoreBoardIndexEnum,
    ) {
    ...
    adjustPlayerScoreBoardRow(
        playerId: number,
        adjustments: ScoreBoardAdjustment[],
    ) {
        ...
        for (let i = 0; i < adjustments.length; i++) {
            ...
            this.scoreBoard[playerId][adjustment.scoreBoardIndex] +=
                adjustment.change;

            if (adjustment.scoreBoardIndex <= ScoreBoardIndexEnum.Imperial) {
                this.scoreBoardAvailable[adjustment.scoreBoardIndex] -=
                    adjustment.change;
            }
        }
    }

    adjustScoreBoardColumn(
        scoreBoardIndex: ScoreBoardIndexEnum,
        adjustments: number[],
    ) {
        this.scoreBoard = [...this.scoreBoard];
    ...
    updateNetWorths() {
        ...
        const netWorths = this.getScoreBoardColumnArray(
