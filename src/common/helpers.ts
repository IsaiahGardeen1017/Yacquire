...
export function calculateBonuses(sharesOwned: number[], sharePrice: number) {
    ...
    for (let playerId = 0; playerId < sharesOwned.length; playerId++) {
        if (sharesOwned[playerId] > 0) {
            playerIdAndAmountArray.push(
                new PlayerIdAndAmount(playerId, sharesOwned[playerId]),
            );
        }
    }
    ...
    if (playerIdAndAmountArray.length === 0) {
        ...
        bonuses.push(
            new PlayerIdAndAmount(
                playerIdAndAmountArray[0].playerId,
                bonusPrice + bonusPrice / 2,
            ),
        );
    } else if (
        playerIdAndAmountArray[0].amount === playerIdAndAmountArray[1].amount
    ) {
        ...
        while (
            ...
            playerIdAndAmountArray[numTying].amount ===
                playerIdAndAmountArray[0].amount
        ) {
            numTying++;
        }
        ...
        for (let i = 0; i < numTying; i++) {
            bonuses.push(
                new PlayerIdAndAmount(
                    playerIdAndAmountArray[i].playerId,
                    bonus,
                ),
            );
        }
        ...
        while (
            ...
            playerIdAndAmountArray[numTying + 1].amount ===
                playerIdAndAmountArray[1].amount
        ) {
            numTying++;
        }
        ...
        if (numTying === 1) {
            ...
            bonuses.push(
                new PlayerIdAndAmount(
                    playerIdAndAmountArray[1].playerId,
                    bonusPrice / 2,
                ),
            );
        } else {
...
export function concatenateUint8Arrays(arrays: Uint8Array[]) {
    ...
    const totalLength = arrays.reduce(
        (soFar, array) => soFar + array.length,
        0,
    );
