...
export class GameSetup {
    ...
    constructor(
        ...
        if (initialUsers !== undefined) {
            ...
            this.usersSet = new Set(
                initialUsers.filter((user) => user !== null),
            );
        } else {
    ...
    addUser(user: User) {
        ...
        for (let position = 0; position < this.users.length; position++) {
            if (this.users[position] === null) {
                ...
                this.approvals =
                    defaultApprovals[gameModeToNumPlayers.get(this.gameMode)!];
        ...
        for (let position = 0; position < this.users.length; position++) {
            if (this.users[position] === user) {
                if (this.approvals[position] === false) {
                    ...
                    const approvedByEverybody =
                        this.approvals.indexOf(false) === -1;
                    if (approvedByEverybody) {
                        ...
                        if (this.finalUsers !== this.users) {
                            gameSetupChange.userApprovedOfGameSetup!
                                .finalUserIds = this.finalUsers.map(
                                    (user) => user.id,
                                );
                        }
                    }
    ...
    changeGameMode(gameMode: PB_GameMode) {
        ...
        if (newNumPlayers !== oldNumPlayers) {
            ...
            if (newNumPlayers > oldNumPlayers) {
                ...
                for (
                    let oldPosition = oldNumPlayers - 1;
                    oldPosition >= newNumPlayers;
                    oldPosition--
                ) {
                    if (users[oldPosition] !== null) {
                        for (
                            let newPosition = newNumPlayers - 1;
                            newPosition >= 0;
                            newPosition--
                        ) {
    ...
    processChange(gameSetupChange: PB_GameSetupChange) {
        ...
        if (gameSetupChange.userAdded) {
            this.addUser(
                this.userIdToUser.get(gameSetupChange.userAdded.userId)!,
            );
        } else if (gameSetupChange.userRemoved) {
            this.removeUser(
                this.userIdToUser.get(gameSetupChange.userRemoved.userId)!,
            );
        } else if (gameSetupChange.userApprovedOfGameSetup) {
            this.approve(
                this.userIdToUser.get(
                    gameSetupChange.userApprovedOfGameSetup.userId,
                )!,
            );
            ...
            if (gameSetupChange.userApprovedOfGameSetup.approvedByEverybody) {
                if (
                    gameSetupChange.userApprovedOfGameSetup.finalUserIds
                        .length > 0
                ) {
                    this.finalUsers = gameSetupChange.userApprovedOfGameSetup
                        .finalUserIds.map(
            ...
            this.kickUser(
                this.userIdToUser.get(gameSetupChange.userKicked.userId)!,
            );
        }
    }
    ...
    private getFinalUsers(): User[] {
        ...
        if (
            ...
            for (
                ...
                playerIndexInTeam < numPlayersPerTeam;
                playerIndexInTeam++
            ) {
